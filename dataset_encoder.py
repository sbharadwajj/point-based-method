...
class Kitti360(data.Dataset): 
    def __init__(self, train = True, npoints = 8192):
        if train:
            ...
            self.Y = {"106.dat":0, "22.dat":1, "269.dat":2, "370.dat":3, "429.dat":4, "555.dat":5, "670.dat":6, "750.dat":7}
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            target = self.Y[model_id]
        else:
            partial =read_pcd(os.path.join(self.inp_val, model_id), center)
            # one_hot[torch.arange(1), self.Y[model_id]] = 1
            target = self.Y[model_id]
        return model_id, resample_pcd(partial, 1024), target#one_hot
